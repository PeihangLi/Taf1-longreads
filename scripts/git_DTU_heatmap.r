...
iso <- isoformProportions(norm_exp)

# Extract isoform proportion from aim regions(DTU data prep)
iso_proportions <- assays(iso)$isoProp

# Generate heatmap of average transcript usage across all tissues(aggregated over sex)
  dtu_heatmap <- iso_proportions %>%
  as.data.frame() %>%
...
# Prepare isoform proportion data
dtu_heatmap <- iso_proportions %>%
  as.data.frame() %>%
  rownames_to_column('txp') %>% 
  reshape2::melt() %>% 
  ...
  scale_fill_gradientn(
    colors = c("#F5F5F5", "#CD4F39"),
    values = scales::rescale(c(0, 1)),
    na.value = "grey90",
    name = "Isoform fraction"
  ) +
...
library(svglite)
svglite("/Users/lipeihang/Desktop/PaperFigure/heat_dtu.svg", width = 8, height = 6, bg = "white")
print(dtu_heatmap)
dev.off()
